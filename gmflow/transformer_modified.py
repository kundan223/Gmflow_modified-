...
def generate_shift_window_attn_mask(
    ...
    print(" h and w for attn_mask:" , h,w )
...
def single_head_split_window_attention(
    ...
    print("h , w and q.size =" , h ,w ,q.shape)
    assert q.size(1) == h * w
    ...
    if with_slot == True:
        ...
        if with_shift:
            print("score dimenshion", scores.shape)
...
class DownsampleBlock(nn.Module):
    ...
    def forward(self, x , h , w ):
            x = x.to(device)
            batch_size, h_w, channels = x.shape
            H = h
            W = w
            ...
            x = self.norm(x)
            x = self.relu(x)
            batch_size , channels ,h_1, h_2 = x.shape 
            x = x.view(batch_size, h_1*h_2 , channels)
            return x , h_1 , h_2 
    ...
    def forward(
        ...
        **kwargs,
        ...
        if self.attention_type == "swin" and attn_num_splits > 1:
            ...
            window_size_h_1 = h // (attn_num_splits*2)
            window_size_w_1 = w // (attn_num_splits*2)
            ...
            print("shifted_window_attn_mask_shape:" , shifted_window_attn_mask.shape)
        ...
        concat1 = torch.cat((feature1, feature0), dim=0)  # [2B, H*W, C]
         # Determine the number of channels from the tensor dimensions
       
        num_channels = concat0.shape[2]
                
        downsample = DownsampleBlock(in_channels=num_channels, out_channels=num_channels).to(device)
        ...
        for i, layer in enumerate(self.layers):
            ...
            if i <= 3:
                ...
                if i == 4:
                    concat0, h_1, w_1 = downsample(concat0, h, w)
                    concat1, _, _ = downsample(concat1, h, w)
                    print("ith loop:" , i)
                    print("concat0 shape after downsample:", concat0.shape)
                    print("Updated h_1 and w_1:", h_1, w_1)
                    ...
                    concat0, _,_ = downsample(concat0, h_1, w_1)
                    concat1,  h_1, w_1 = downsample(concat1, h_1, w_1)
                    
                    print("ith loop:" , i)
                    print("concat0 shape after downsample:", concat0.shape,i)
                    print("Updated h_1 and w_1:", h_1, w_1)
        ...
        # reshape back
        feature0 = (
            feature0.view(b, h_1, w_1, c).permute(0, 3, 1, 2).contiguous()
        )  # [B, C, H, W]
        feature1 = (
            feature1.view(b, h_1, w_1, c).permute(0, 3, 1, 2).contiguous()
        )  # [B, C, H, W]
